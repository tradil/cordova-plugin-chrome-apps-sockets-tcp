...
- (void)connect:(CDVInvokedUrlCommand*)command
{
	...
	NSTimeInterval timeout = [[command argumentAtIndex:3] integerValue];
    ...
    BOOL success = FALSE;
	if (timeout > 0)
		success = [socket->_socket connectToHost:peerAddress onPort:peerPort withTimeout:timeout error:&err];
	else
		success = [socket->_socket connectToHost:peerAddress onPort:peerPort error:&err];
    if (!success) {
